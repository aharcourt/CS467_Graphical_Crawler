...
module.exports =
{
   ...
   doesSearchExist: function doesSearchExist(cookie, usr, stype, sdepth, root, kwtxt)
   {
	   ...
	   {
           // Run the query
           pool.query(queryString, [ cookie, usr, stype, sdepth, root, kwtxt ], function(err, rows, fields)
		   {
               if(err)
			   {
                   reject(err);
	...
	*/
	...
	insertIntoTempEdges: function insertIntoTempEdges(js, sUrl, sTitle, dUrl, dTitle)
	{
			console.log("Inserting temp edge", ...arguments);
	   // Locals
	   ...
	   {
		   // Run the query
		   pool.query(queryString, [ js, sUrl, sTitle, dUrl, dTitle ], function(err, rows, fields)
		   {
			   if(err)
			   {
				   reject(err);
	...
	insertNewTree: function insertNewTree(js, root, user, cookie, srchtype, srchdepth, kwtxt, kwurl)
	{
	   // Locals
	   ...
	   {
		   // Run the query
		   pool.query(queryString, [ js, root, user, cookie, srchtype, srchdepth, kwtxt, kwurl ], function(err, rows, fields)
		   {
			   if(err)
			   {
				   reject(err);
	...
	getExistingTree: function getExistingTree(cookie, user, stype, sdepth, root, kwtxt)
	{
	   // Locals
	   ...
	   {
		   // Run the query
		   pool.query(queryString, [ cookie, user, stype, sdepth, root, kwtxt ], function(err, rows, fields)
		   {
			   if(err)
			   {
				   reject(err);
