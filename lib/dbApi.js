...
module.exports =
{
	...
	*/
	...
	/* Find the id for an existing user to use as a cookie */
	getUserID: function getUserID(name) {
		let queryString = "SELECT Id FROM wc_Users WHERE Name = ?";
		return new Promise((resolve, reject) => {
			pool.query(queryString, [ name ], function(err, rows, fields) {
				if (err) {
					reject(err);
					return;
				}
				let data = JSON.parse(JSON.stringify(rows));
				let result = (data[0] == null) ? null : data[0].Id;
				resolve(result);
			})
		});
	},

	/* Find previous searches a user has performed */
	getPreviousSearches: function getPreviousSearches(userID) {
		let queryString = "SELECT * FROM wc_UserSearches WHERE CookieID = ?";
		return new Promise((resolve, reject) => {
			pool.query(queryString, [ userID ], function(err, rows, fields) {
				if (err) {
					reject(err);
					return;
				}
				resolve(JSON.parse(JSON.stringify(rows))); // TODO: get better data
			})
		});
	}
};
