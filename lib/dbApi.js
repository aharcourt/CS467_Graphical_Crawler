...
module.exports = 
{	
   // Wrapper for wcSp_DoesCookieExist(IN cookie int, IN user varchar(255),
	// IN srchtype varchar(255), IN srchdepth int, IN root text, OUT result int)
   // Checks for the presence of this search in the wc_UserSearches table.
   doesSearchExist: function doesSearchExist(cookie, usr, stype, sdepth, root)   
   {
	   ...
	   {
           // Run the query
           pool.query(queryString, [ cookie, usr, stype, sdepth, root ], function(err, rows, fields) 
		   {
               if(err) 
			   {
                   reject(err);
	...
	// Wrapper for wcSp_InsertIntoTempEdges(IN s text, IN d text, IN cookie int, IN username varchar(255),
	// IN stype varchar(255), IN sdepth int, IN root text, OUT result int)
	...
	insertIntoTempEdges: function insertIntoTempEdges(sUrl, dUrl, cookie, user, stype, sdepth, root)   
	{
	   // Locals
	   ...
	   {
		   // Run the query
		   pool.query(queryString, [ sUrl, dUrl, cookie, user, stype, sdepth, root ], function(err, rows, fields) 
		   {
			   if(err) 
			   {
				   reject(err);
	...
	// Wrapper for wcSp_InsertNewTree(IN root text, IN user varchar(255), IN cookie int, IN srchtype varchar(255), srchdepth int, OUT result int)
	...
	insertNewTree: function insertNewTree(root, user, cookie, srchtype, srchdepth)   
	{
	   // Locals
	   ...
	   {
		   // Run the query
		   pool.query(queryString, [ root, user, cookie, srchtype, srchdepth ], function(err, rows, fields) 
		   {
			   if(err) 
			   {
				   reject(err);
	...
	// Wrapper for wcSp_GetExistingTree(IN cookie int, IN user varchar(255), 
	// IN srchtype varchar(255), IN srchdepth int, IN root text)
	// Returns set of records for a specific user search identified by a specific cookie
	getExistingTree: function getExistingTree(cookie, user, stype, sdepth, root)   
	{
	   // Locals
	   ...
	   {
		   // Run the query
		   pool.query(queryString, [ cookie, user, stype, sdepth, root ], function(err, rows, fields) 
		   {
			   if(err) 
			   {
				   reject(err);
