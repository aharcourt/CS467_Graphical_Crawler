...
let addToDB = function(returnVal, url, cookie, searchType, searchDepth) {
    let dbPromise = new Promise((resolve, reject) => {
        let treeInsert = dbAPI.insertNewTree(url, 'Ian Dalrymple', cookie, searchType, searchDepth);
                
        treeInsert.then((result) => {
            resolve(returnVal);
            return;
        });
        treeInsert.catch((err) => {
            throw err;
        });
    });
...
let crawl = function(searchType, rootURL, searchDepth, cookie, searchExists){
    let crawlPromise = new Promise((resolve, reject) => {
        if (searchExists){
            resolve(1);
            return;
        }
        else if (searchType == 'BFS') {
            let BFS = breadthFS(rootURL, searchDepth, cookie);
            
            BFS.then((result) => {
                resolve(addToDB(result, rootURL, cookie, 'BFS', searchDepth));
                return;                
            });  
            BFS.catch((err) => {
                throw err;
            ...
            let DFS = depthFS(rootURL, searchDepth, cookie);
            
            DFS.then((result) => {
                resolve(addToDB(result, rootURL, cookie, 'DFS', searchDepth));
                return;                
            });  
            DFS.catch((err) => {
                throw err;
