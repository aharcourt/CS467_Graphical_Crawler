...
let breadthFS = function (rootURL, searchDepth, cookie) {
    let currentDepth = 0;
    ...
    let searchingPromise = new Promise((resolve, reject) => {
        ...
        function getLinks(url, level, sourceTitle, sourceURL) {
            ...
            fetch.then((body) => {
                ...
                console.log("Link <title>", destTitle); 

                 // Insert link into temp edges table
                let tempInsert = dbAPI.insertIntoTempEdges(sourceURL, sourceTitle, url, destURL);   
                ...
                if (level < searchDepth) {
                    ...
                    $(links).each(function(i, link){
                        ...
                        if(linkUrl != "javascript:void(0)" && validUrl.isUri(linkUrl)){
                            getLinks(linkUrl, level + 1, title, url);
                        }
                    });
                }
            }).catch((err) =>{});
...
let addToDB = function(returnValue, url, cookie, searchType, searchDepth) { 
  return dbAPI.insertNewTree(url, 'Ian Dalrymple', cookie, searchType, searchDepth).then(() => { 
    return returnVal; 
  }); 
};
...
let crawl = function(searchType, rootURL, searchDepth, cookie, searchExists){
    if (searchExists) {
      return Promise.resolve(1);
    } else if (searchType === "BFS") {
        return breadthFS(rootUrl, searchDepth, cookie).then((result) => {
            return addToDB(result, rootUrl, cookie, "BFS", searchDepth);
        });
    } else if (searchType === "DFS") {
        return depthFS(rootUrl, searchDepth, cookie).then((result) => {
             return addToDB(result, rootUrl, cookie, "DFS", searchDepth);
        });
    } else {
        return Promise.resolve(-1);
    }
