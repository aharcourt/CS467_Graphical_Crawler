...
let depthFS = function (rootUrl, searchDepth, user, keyword) {
    let currentDepth = 0;
    let edges = [];
    let nextUrl = rootUrl;
    let rootTitle;
...
let addToDB = function(result, url, user, searchType, searchDepth, keyword) {
    return dbAPI.insertNewTree(url, user.name, user.id, searchType, searchDepth, keyword, result.keywordURL).then(() => {
        return result;
    });
};

// Routes to proper search function, resolves to -1 if the search is malformed.
let crawl = function(searchType, rootURL, searchDepth, user, keyword) {
    if (searchType === "BFS") {
        return breadthFS(rootURL, searchDepth, user, keyword).then((result) => {
            return addToDB(result, rootURL, user, "BFS", searchDepth, keyword);
        });
    } else if (searchType === "DFS") {
        return depthFS(rootURL, searchDepth, user, keyword).then((result) => {
            return addToDB(result, rootURL, user, "DFS", searchDepth, keyword);
        });
    } else {
