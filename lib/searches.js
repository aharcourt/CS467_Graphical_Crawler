...
let getDFSLinks = function (sourceLinks, level, searchDepth, searchID, attempts, result, keyword) {
    ...
    let randomLink = sourceLinks[Math.floor(Math.random() * sourceLinks.length)];

    let helperPromise = request({
        url: randomLink.DestinationURL,
        timeout: 30000,
        jar: true
    });
    return helperPromise.then((body) => {
        // if the body is too large cheerio can't laod it. Treat it like a fetch error.
        if (body.length > 32000000) {
            throw new Error("Body is waaay too long!");
        }
        try {
            let destLinks = [];
